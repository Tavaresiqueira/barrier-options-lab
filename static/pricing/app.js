...
function singleBarrierLearningMarkup(result) {
  ...
  return `<section class="embedded-scenarios single-barrier-learning">
    ...
    <article class="embedded-chart-card"><header><span>VALUATION ${scenario.valuation_date} · EXPIRY ${scenario.expiration_date}</span><strong>Current exotic versus vanilla value per option unit</strong><small>${contract.option_type.toUpperCase()} · K R$ ${money(contract.strike)} · barrier R$ ${money(contract.barrier)} · ${scenario.paths_used.toLocaleString("pt-BR")} paths per exotic point.</small></header><div class="embedded-canvas"><canvas data-single-barrier-chart></canvas></div><div class="vanilla-comparison-strip"><div><span>Barrier-option premium</span><strong>R$ ${money(scenario.initial_exotic_premium_per_unit)}</strong></div><div><span>Same vanilla ${contract.option_type}</span><strong>R$ ${money(scenario.initial_vanilla_premium_per_unit)}</strong></div><div class="premium-gap"><span>Premium discount from barrier</span><strong>R$ ${money(scenario.initial_premium_discount_per_unit)}</strong><small>${(scenario.initial_premium_discount_per_unit/scenario.initial_vanilla_premium_per_unit*100).toFixed(1)}% below vanilla</small></div></div>${singleBarrierSpotMarkup(result)}${singleBarrierEquivalenceMarkup()}<p>The purple vanilla line has no barrier event. The green exotic branch stops before the trigger and the dashed red post-trigger branch begins at the barrier, making the event jump explicit.</p></article>
  </section>`;
}
