...
function singleBarrierSpotMarkup(result) {
  ...
  const baseVolPct = Number(result.contract_snapshot.volatility) * 100, minimumShock = Math.max(-20, -baseVolPct + .25);
  return `<section class="spot-explorer" data-barrier-spot-explorer><div class="spot-slider-copy"><small>LIVE OPTION REPRICE</small><strong>Spot <output data-spot-output>R$ ${money(spot)}</output></strong><label class="manual-spot-field"><span>Type exact spot</span><span><b>R$</b><input data-spot-manual type="number" min="0.01" value="${spot}" step="0.01" inputmode="decimal" aria-label="Exact spot price"></span></label><span>Move spot and shock volatility to recalculate the unit premium and Greeks with all other contract terms fixed.</span></div><div class="live-shock-controls"><label><span>Spot scenario</span><input data-spot-slider type="range" min="${low}" max="${high}" value="${spot}" step="${Math.max((high-low)/200,.01)}"></label><label class="vol-shock-control"><span>Volatility shock <b data-vol-shock-output>0.00 pp</b> · resulting vol <strong data-vol-result-output>${baseVolPct.toFixed(2)}%</strong></span><input data-vol-shock type="range" min="${minimumShock}" max="20" value="0" step="0.25"></label></div><div class="spot-snapshot" data-spot-snapshot></div></section>`;
}
...
function attachSingleBarrierLearning(target, result) {
  ...
  const slider=$("[data-spot-slider]",explorer), manual=$("[data-spot-manual]",explorer), output=$("[data-spot-output]",explorer), volSlider=$("[data-vol-shock]",explorer), volShockOutput=$("[data-vol-shock-output]",explorer), volResultOutput=$("[data-vol-result-output]",explorer), snapshot=$("[data-spot-snapshot]",explorer); let timer,sequence=0,selectedSpot=Number(result.contract_snapshot.spot); const baseVolatility=Number(result.contract_snapshot.volatility); renderSingleBarrierSnapshot(snapshot,result);
  const scheduleReprice=()=>{const shockPp=Number(volSlider.value),volatility=baseVolatility+shockPp/100;volShockOutput.textContent=`${shockPp>0?"+":""}${shockPp.toFixed(2)} pp`;volResultOutput.textContent=`${(volatility*100).toFixed(2)}%`;clearTimeout(timer);timer=setTimeout(async()=>{const id=++sequence,payload={...saved.payload,spot:selectedSpot,volatility,calculate_greeks:true};if((payload.direction==="up"&&selectedSpot>=payload.barrier)||(payload.direction==="down"&&selectedSpot<=payload.barrier))payload.barrier_status="triggered";snapshot.classList.add("loading");try{const repriced=await postJSON("/api/v1/barriers/snapshot/",payload);if(id===sequence)renderSingleBarrierSnapshot(snapshot,repriced);}catch(error){if(id===sequence)snapshot.innerHTML=`<span class="error">${errorText(error)}</span>`;}finally{snapshot.classList.remove("loading");}},260);};
  const repriceSpot=spot=>{if(!Number.isFinite(spot)||spot<=0){manual.setCustomValidity("Enter a spot greater than zero.");manual.reportValidity();return;}manual.setCustomValidity("");selectedSpot=spot;manual.value=spot;slider.value=Math.max(Number(slider.min),Math.min(Number(slider.max),spot));output.textContent=`R$ ${money(spot)}`;scheduleReprice();};
  ...
  volSlider.oninput=scheduleReprice;
